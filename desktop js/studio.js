...
class ImporterFile {
  ...
  upload(passedname) {
    let name = passedname;
    if (name == "") name = "unnamed" + Math.random().toString().substring(2, 8); // set the importer icon
    ...
    $.ajax({
      ...
      if (d.status == "ok") {
        ...
        d.data.thumbnail = thumbUrl; // alert the studio

        studio[0].importerStatus("done");
        studio[0].importerUploadComplete(importType, d.data.file, d.data); // update html for images

        if (this.subtype == 0) {
          if (this.ext != "swf") this.el.find("img").attr("src", `/assets/${d.data.file}`); // change the subtypes to an add to scene button
      ...
      } else alert("Error importing asset."); // remove element
